...
use crate::{
    editor::{
        annotated_string::{annotated_type::AnnotationType, AnnotatedString},
        line::{grapheme_width::GraphemeWidth, text_fragment::TextFragment},
    },
    types::{ByteIndex, ColIndex, GraphemeIndex},
};
use std::{
    cmp::min,
    fmt,
...
impl Line {
    ...
    pub fn get_annotated_visible_substr(
        ...
        // Inserting replacement chars and trucating if needed.
        // Doing it backwards, or the byte indices would be off in case of a replacement character
        // has a different width than the original
    ...
    fn byte_idx_to_grapheme_idx(&self, byte_idx: ByteIndex) -> Option<GraphemeIndex> {
    ...
    fn grapheme_idx_to_byte_idx(&self, grapheme_idx: GraphemeIndex) -> ByteIndex {
